...
class Category_model extends CI_Model {
    ...
    public function listCategoryTree() {
        ...
        return '<ul class="vmagicmenu-narrow clearfix">'.$this->getCategories(0, $category).'</ul>';
    }

    function getCategories($parent, $category,$level=0) {
        $html = '';
        if (isset($category['parent_cats'][$parent])) {
            foreach ($category['parent_cats'][$parent] as $cat_id) {
                $html .= '<li class="level'.$level.''.($level==0 ? ' cat':'').''.(isset($category['parent_cats'][$cat_id]) ? ' hasChild dropdown':'').'">
                            <a'.($level==0 ? ' class="level-top"':'').' href="http://alothemes.com/demo/supermarket/index.php/automovie-motorcyle.html">
