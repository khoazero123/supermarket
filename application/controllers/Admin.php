...
class Admin extends CI_Controller {
	...
	public function addProduct($type='add') {
		//$insert['id'] = $this->input->post('product-id'); // :6825
		...
		if($id=$this->Product_model->insertProduct($insert)) {
			$data['alert'] = ['message'=>'Add product success! #<a href="'.base_url().'admin/page_ecom_product_edit.php?id='.$id.'">'.$id.'</a>'];
		} else {
	...
	public function editProduct($id) {
		//redirect('admin/'.$this->views.'.php?id='.$id);
		//redirect($this->uri->uri_string());
		//if($this->input->get('type') == 'add_images') return $this->uploadImage();

		//$images = $this->session->userdata('tmp_upload_img');
		//if(!empty($images)) $update['images'] = $images;
		...
		if($this->Product_model->updateProduct($update,$id)) {
			$data['alert'] = ['message'=>'Edit product success! #<a href="'.base_url().'admin/page_ecom_product_edit.php?id='.$id.'">'.$id.'</a>'];
		} else {
